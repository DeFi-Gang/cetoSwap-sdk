...
import { CurrencyAmount, Percent, Pulse, Token } from '@pulsex/sdk-core'
import { WETH, ETHER } from './constants'
...
describe('Router', () => {
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WETH[169], '1000'),
    ...
    describe('exact in', () => {
      it.only('ether to token1', () => {
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WETH[169].address, token0.address, token1.address],
      ...
      it('deadline specified', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(
            new Route([pair_weth_0, pair_0_1], ETHER, token1),
            CurrencyAmount.fromRawAmount(Pulse.onChain(169), JSBI.BigInt(100))
          ),
          {
            deadline: 50,
      ...
      it('token1 to ether', () => {
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WETH[169].address],
          '0x0000000000000000000000000000000000000004'
        ])
